...
var operationFromURL = exports.operationFromURL = function (path) {
  var purl = url.parse(path);
  return purl.pathname.substr(WSAPI_PREFIX.length) || null;
};

var apis = exports.apis = function () {
  if (wsapis) return wsapis;
  ...
  fs.readdirSync(path.join(__dirname, 'wsapi')).forEach(function (f) {
    ...
    try {
      ...
      if ((options.only_write_apis && !api.writes_db && operation != 'ping') ||
          (!options.only_write_apis && api.writes_db && !api.disallow_forward))
            return;
...
exports.setupRoute = function (app, options) {
  var wsapis = apis();
